...
def setRadioLoraRange(lorarange):
    DatabaseHelper.reInit()
    sd = DatabaseHelper.get_setting_by_key('LoraRange')
    if sd is None:
        sd = SettingData()
...
def getRadioAcknowledgementRequested():
    DatabaseHelper.reInit()
...
def setRadioAcknowledgement(ack):
    DatabaseHelper.reInit()
