...
class DatabaseHelper:
    ...
    def set_ack_received_from_receiver_on_repeater_lora_ack_message_subscription(cls, messageID: bytearray) -> None:
        ...
        sql = ("SELECT MessageSubscriptionData.* FROM MessageSubscriptionData JOIN SubscriptionData "
               ...
               "WHERE TransformData.Name IN ('RepeaterSIMessageToLoraAckTransform', 'RepeaterSIMessageDoubleToLoraAckTransform') AND "
    ...
    def get_message_subscriptions_view_to_archive(cls, limit: int = 100):
        ...
        if cnt > 0:
            ...
            sql = ("SELECT MessageSubscriptionData.id, "
                   ...
                   "((MessageSubscriptionData.NoOfSendTries >= SubscriptionData.MaxTries AND (MessageSubscriptionData.SentDate < ? or MessageSubscriptionData.SendFailedDate < ?)) or MessageSubscriptionData.FindAdapterTries >= 5) "
                   "ORDER BY MessageBoxData.CreatedDate desc "
                   "LIMIT %s") % limit
            return cls.db.get_table_objects_by_SQL(MessageSubscriptionView, sql, (fifteenSecondsAgo, fifteenSecondsAgo))
        return []
    ...
    def get_test_punches(cls, testBatchGuid: str, msgBoxId: int | None) -> list[TestPunchView]:
        ...
        SELECT * FROM (
        SELECT 
            TestPunchData.id || '_' || ifnull(ms.id,'') as id, 
            TestPunchData.id as TestPunchId,
            ...
            CASE 
                ...
                WHEN mb.id is not null and ms.id is not null and ms.SentDate is not null and ms.AckReceivedDate is null and SubscriberData.TypeName IN ('SIRAP', 'ROC', 'SERIAL', 'SRR') THEN 'Sent' 
        ...
        SELECT 
            ...
            null as TestPunchId,
            CASE
               WHEN mb2.MessageTypeName = 'STATUS' THEN 'STATUS'
               ELSE mb2.SICardNumber
            END SICardNumber,
            CASE 
                WHEN mb2.SportIdentHour is not null and cast(mb2.SportIdentHour as integer) >= 12 THEN 1 
                ELSE 0 
            END TwentyFourHour, 
            CASE 
                WHEN mb2.SportIdentHour is null THEN 0
                WHEN cast(mb2.SportIdentHour as integer)>= 12 THEN (cast(mb2.SportIdentHour as integer)-12) * 3600 + cast(mb2.SportIdentMinute as integer) * 60 + cast(mb2.SportIdentSecond as integer) 
                ELSE cast(mb2.SportIdentHour as integer) * 3600 + cast(mb2.SportIdentMinute as integer) * 60 + cast(mb2.SportIdentSecond as integer) 
            END TwelveHourTimer, 
            False as Fetched, 
            ...
            CASE 
                ...
                WHEN mb2.id is not null and ms2.id is not null and ms2.SentDate is not null and ms2.AckReceivedDate is null and SubscriberData.TypeName IN ('SIRAP', 'ROC', 'SERIAL', 'SRR') THEN 'Sent' 
    ...
    def add_default_channels(cls):
        ...
        if not cls.get_channels_exists():
            channels = [ChannelData('1', 73, 'UL', 439712500, 12, 5, True, True, 8, "DRF1268DS"),
                        ...
                        ChannelData('1', 781, 'MF', 439712500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('2', 781, 'MF', 439762500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('3', 781, 'MF', 439812500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('4', 781, 'MF', 439862500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('5', 781, 'MF', 439912500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('6', 781, 'MF', 439962500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('1', 1367, 'F', 439712500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('2', 1367, 'F', 439762500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('3', 1367, 'F', 439812500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('4', 1367, 'F', 439862500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('5', 1367, 'F', 439912500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('6', 1367, 'F', 439962500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('1', 2344, 'XF', 439712500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('2', 2344, 'XF', 439762500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('3', 2344, 'XF', 439812500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('4', 2344, 'XF', 439862500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('5', 2344, 'XF', 439912500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('6', 2344, 'XF', 439962500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('1', 3906, 'UF', 439712500, 5, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('2', 3906, 'UF', 439762500, 5, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('3', 3906, 'UF', 439812500, 5, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('4', 3906, 'UF', 439862500, 5, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('5', 3906, 'UF', 439912500, 5, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('6', 3906, 'UF', 439962500, 5, 5, True, True, 8, "DRF1268DS"),
                        ...
                        ChannelData('HAM1', 781, 'MF', 433737500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM2', 781, 'MF', 433787500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM3', 781, 'MF', 433837500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM4', 781, 'MF', 433887500, 8, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM5', 781, 'MF', 433937500, 8, 5, True, True, 8, "DRF1268DS"),

                        ChannelData('HAM1', 1367, 'F', 433737500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM2', 1367, 'F', 433787500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM3', 1367, 'F', 433837500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM4', 1367, 'F', 433887500, 7, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM5', 1367, 'F', 433937500, 7, 5, True, True, 8, "DRF1268DS"),

                        ChannelData('HAM1', 2344, 'XF', 433737500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM2', 2344, 'XF', 433787500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM3', 2344, 'XF', 433837500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM4', 2344, 'XF', 433887500, 6, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM5', 2344, 'XF', 433937500, 6, 5, True, True, 8, "DRF1268DS"),

                        ChannelData('HAM1', 3906, 'UF', 433737500, 5, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM2', 3906, 'UF', 433787500, 5, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM3', 3906, 'UF', 433837500, 5, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM4', 3906, 'UF', 433887500, 5, 5, True, True, 8, "DRF1268DS"),
                        ChannelData('HAM5', 3906, 'UF', 433937500, 5, 5, True, True, 8, "DRF1268DS"),
                        ...
                        ChannelData('1', 781, 'MF', 439712500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('2', 781, 'MF', 439762500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('3', 781, 'MF', 439812500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('4', 781, 'MF', 439862500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('5', 781, 'MF', 439912500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('6', 781, 'MF', 439962500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('1', 1367, 'F', 439712500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('2', 1367, 'F', 439762500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('3', 1367, 'F', 439812500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('4', 1367, 'F', 439862500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('5', 1367, 'F', 439912500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('6', 1367, 'F', 439962500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('1', 2344, 'XF', 439712500, 6, 7, False, True, 8,"RAK3172"),
                        ChannelData('2', 2344, 'XF', 439762500, 6, 7, False, True, 8,"RAK3172"),
                        ChannelData('3', 2344, 'XF', 439812500, 6, 7, False, True, 8,"RAK3172"),
                        ChannelData('4', 2344, 'XF', 439862500, 6, 7, False, True, 8,"RAK3172"),
                        ChannelData('5', 2344, 'XF', 439912500, 6, 7, False, True, 8,"RAK3172"),
                        ChannelData('6', 2344, 'XF', 439962500, 6, 7, False, True, 8,"RAK3172"),
                        ChannelData('1', 3906, 'UF', 439712500, 5, 7, False, True, 8,"RAK3172"),
                        ChannelData('2', 3906, 'UF', 439762500, 5, 7, False, True, 8,"RAK3172"),
                        ChannelData('3', 3906, 'UF', 439812500, 5, 7, False, True, 8,"RAK3172"),
                        ChannelData('4', 3906, 'UF', 439862500, 5, 7, False, True, 8,"RAK3172"),
                        ChannelData('5', 3906, 'UF', 439912500, 5, 7, False, True, 8,"RAK3172"),
                        ChannelData('6', 3906, 'UF', 439962500, 5, 7, False, True, 8,"RAK3172"),
                        ...
                        ChannelData('HAM1', 781, 'MF', 433737500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('HAM2', 781, 'MF', 433787500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('HAM3', 781, 'MF', 433837500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('HAM4', 781, 'MF', 433887500, 8, 7, False, True, 8,"RAK3172"),
                        ChannelData('HAM5', 781, 'MF', 433937500, 8, 7, False, True, 8,"RAK3172"),

                        ChannelData('HAM1', 1367, 'F', 433737500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('HAM2', 1367, 'F', 433787500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('HAM3', 1367, 'F', 433837500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('HAM4', 1367, 'F', 433887500, 7, 7, False, True, 8,"RAK3172"),
                        ChannelData('HAM5', 1367, 'F', 433937500, 7, 7, False, True, 8,"RAK3172"),

                        ChannelData('HAM1', 2344, 'XF', 433737500, 6, 7, False, False, 8,"RAK3172"),
                        ChannelData('HAM2', 2344, 'XF', 433787500, 6, 7, False, False, 8,"RAK3172"),
                        ChannelData('HAM3', 2344, 'XF', 433837500, 6, 7, False, False, 8,"RAK3172"),
                        ChannelData('HAM4', 2344, 'XF', 433887500, 6, 7, False, False, 8,"RAK3172"),
                        ChannelData('HAM5', 2344, 'XF', 433937500, 6, 7, False, False, 8,"RAK3172"),

                        ChannelData('HAM1', 3906, 'UF', 433737500, 5, 7, False, False, 8,"RAK3172"),
                        ChannelData('HAM2', 3906, 'UF', 433787500, 5, 7, False, False, 8,"RAK3172"),
                        ChannelData('HAM3', 3906, 'UF', 433837500, 5, 7, False, False, 8,"RAK3172"),
                        ChannelData('HAM4', 3906, 'UF', 433887500, 5, 7, False, False, 8,"RAK3172"),
                        ChannelData('HAM5', 3906, 'UF', 433937500, 5, 7, False, False, 8,"RAK3172"),

                        # RfBW = 4 => 10.4 kHz (A/B channels overlap with 31 kHz channels of same number)
                        ChannelData('1A', 81, 'UL', 439700000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('1B', 81, 'UL', 439725000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('2A', 81, 'UL', 439750000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('2B', 81, 'UL', 439775000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('3A', 81, 'UL', 439800000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('3B', 81, 'UL', 439825000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('4A', 81, 'UL', 439850000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('4B', 81, 'UL', 439875000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('5A', 81, 'UL', 439900000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('5B', 81, 'UL', 439925000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('6A', 81, 'UL', 439950000, 10, 4, False, True, 8,"RAK3172"),
                        ChannelData('6B', 81, 'UL', 439975000, 10, 4, False, True, 8,"RAK3172"),

                        ChannelData('1A', 146, 'XL', 439700000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('1B', 146, 'XL', 439725000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('2A', 146, 'XL', 439750000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('2B', 146, 'XL', 439775000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('3A', 146, 'XL', 439800000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('3B', 146, 'XL', 439825000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('4A', 146, 'XL', 439850000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('4B', 146, 'XL', 439875000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('5A', 146, 'XL', 439900000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('5B', 146, 'XL', 439925000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('6A', 146, 'XL', 439950000, 9, 4, False, True, 8,"RAK3172"),
                        ChannelData('6B', 146, 'XL', 439975000, 9, 4, False, True, 8,"RAK3172"),

                        ChannelData('1A', 260, 'L', 439700000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('1B', 260, 'L', 439725000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('2A', 260, 'L', 439750000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('2B', 260, 'L', 439775000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('3A', 260, 'L', 439800000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('3B', 260, 'L', 439825000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('4A', 260, 'L', 439850000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('4B', 260, 'L', 439875000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('5A', 260, 'L', 439900000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('5B', 260, 'L', 439925000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('6A', 260, 'L', 439950000, 8, 4, False, True, 8,"RAK3172"),
                        ChannelData('6B', 260, 'L', 439975000, 8, 4, False, True, 8,"RAK3172"),

                        ChannelData('1A', 455, 'ML', 439700000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('1B', 455, 'ML', 439725000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('2A', 455, 'ML', 439750000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('2B', 455, 'ML', 439775000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('3A', 455, 'ML', 439800000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('3B', 455, 'ML', 439825000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('4A', 455, 'ML', 439850000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('4B', 455, 'ML', 439875000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('5A', 455, 'ML', 439900000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('5B', 455, 'ML', 439925000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('6A', 455, 'ML', 439950000, 7, 4, False, False, 8,"RAK3172"),
                        ChannelData('6B', 455, 'ML', 439975000, 7, 4, False, False, 8,"RAK3172"),

                        ChannelData('1A', 781, 'MF', 439700000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('1B', 781, 'MF', 439725000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('2A', 781, 'MF', 439750000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('2B', 781, 'MF', 439775000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('3A', 781, 'MF', 439800000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('3B', 781, 'MF', 439825000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('4A', 781, 'MF', 439850000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('4B', 781, 'MF', 439875000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('5A', 781, 'MF', 439900000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('5B', 781, 'MF', 439925000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('6A', 781, 'MF', 439950000, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('6B', 781, 'MF', 439975000, 6, 4, False, False, 8,"RAK3172"),

                        ChannelData('1A', 1367, 'F', 439700000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('1B', 1367, 'F', 439725000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('2A', 1367, 'F', 439750000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('2B', 1367, 'F', 439775000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('3A', 1367, 'F', 439800000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('3B', 1367, 'F', 439825000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('4A', 1367, 'F', 439850000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('4B', 1367, 'F', 439875000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('5A', 1367, 'F', 439900000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('5B', 1367, 'F', 439925000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('6A', 1367, 'F', 439950000, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('6B', 1367, 'F', 439975000, 5, 4, False, False, 8,"RAK3172"),
                        ...
                        ChannelData('HAM1', 781, 'MF', 433737500, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('HAM2', 781, 'MF', 433787500, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('HAM3', 781, 'MF', 433837500, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('HAM4', 781, 'MF', 433887500, 6, 4, False, False, 8,"RAK3172"),
                        ChannelData('HAM5', 781, 'MF', 433937500, 6, 4, False, False, 8,"RAK3172"),

                        ChannelData('HAM1', 1367, 'F', 433737500, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('HAM2', 1367, 'F', 433787500, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('HAM3', 1367, 'F', 433837500, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('HAM4', 1367, 'F', 433887500, 5, 4, False, False, 8,"RAK3172"),
                        ChannelData('HAM5', 1367, 'F', 433937500, 5, 4, False, False, 8,"RAK3172"),
    ...
    def add_timeonair(cls):
        ...
        if not cls.get_timeonair_exists():
            ...
            timeOnAirDatas = [
                        ...
                        TimeOnAirData(6, 5, 1, True, True, True, 8, True, "DRF1268DS", 138, 200, 97, 138, 118),
                        TimeOnAirData(5, 5, 1, True, True, True, 8, True, "DRF1268DS", 85, 126, 54, 85, 69),
                        ...
                        TimeOnAirData(6, 5, 2, True, True, True, 8, True, "DRF1268DS", 157, 230, 107, 157, 132),
                        TimeOnAirData(5, 5, 2, True, True, True, 8, True, "DRF1268DS", 97, 146, 60, 97, 79),
                        ...
                        TimeOnAirData(6, 5, 3, True, True, True, 8, True, "DRF1268DS", 175, 261, 118, 175, 146),
                        TimeOnAirData(5, 5, 3, True, True, True, 8, True, "DRF1268DS", 109, 167, 66, 109, 88),
                        ...
                        TimeOnAirData(6, 5, 4, True, True, True, 8, True, "DRF1268DS", 193, 292, 128, 193, 161),
                        TimeOnAirData(5, 5, 4, True, True, True, 8, True, "DRF1268DS", 122, 187, 72, 122, 97),

                        # RfBW = 7 => 31,25 kHz on RAK3172, CodeRate 0 = 4/4
                        TimeOnAirData(12, 7, 0, True, True, False, 8, True, "RAK3172", 4620, 6590, 3310, 4620, 3960), #todo: change to compatmode false, adjust numbers accordingly
                        ...
                        TimeOnAirData(6, 7, 0, True, True, False, 8, False, "RAK3172", 107, 159, 77, 118, 97),
                        TimeOnAirData(5, 7, 0, True, True, False, 8, False, "RAK3172", 64, 100, 44, 69, 54),
                        
                        # RAK3172 Non Compat mode
                        # RfBW = 7 => 31,25 kHz on RAK3172, CodeRate 1 = 4/5
                        TimeOnAirData(12, 7, 1, True, True, False, 8,False, "RAK3172", 4620, 5930, 3310, 4620, 3960),
                        TimeOnAirData(11, 7, 1, True, True, False, 8,False, "RAK3172", 2310, 3290, 1980, 2640, 1980),
                        TimeOnAirData(10, 7, 1, True, True, False, 8,False, "RAK3172", 1320, 1810, 992, 1320, 1160),
                        TimeOnAirData(9, 7, 1, True, True, False, 8,False, "RAK3172", 660, 988, 496, 742, 578),
                        TimeOnAirData(8, 7, 1, True, True, False, 8,False, "RAK3172", 371, 535, 289, 412, 330),
                        TimeOnAirData(7, 7, 1, True, True, False, 8,False, "RAK3172", 205, 309, 145, 227, 186),
                        TimeOnAirData(6, 7, 1, True, True, False, 8, False, "RAK3172", 128, 189, 87, 128, 107),
                        TimeOnAirData(5, 7, 1, True, True, False, 8, False, "RAK3172", 74, 115, 49, 80, 64),

                        TimeOnAirData(6, 7, 1, False, True, False, 8, False, "RAK3172", 97, 138, 77, 107, 87),
                        TimeOnAirData(5, 7, 1, False, True, False, 8, False, "RAK3172", 54, 80, 39, 59, 49),

                        # RfBW = 7 => 31,25 kHz on RAK3172, CodeRate 2 = 4/6
                        TimeOnAirData(12, 7, 2, True, True, False, 8, False, "RAK3172", 5010, 6590, 3440, 5010, 4230),
                        TimeOnAirData(11, 7, 2, True, True, False, 8, False, "RAK3172", 2510, 3690, 2110, 2900, 2110),
                        TimeOnAirData(10, 7, 2, True, True, False, 8, False, "RAK3172", 1450, 2040, 1060, 1450, 1250),
                        TimeOnAirData(9, 7, 2, True, True, False, 8, False, "RAK3172", 725, 1120, 529, 824, 627),
                        TimeOnAirData(8, 7, 2, True, True, False, 8, False, "RAK3172", 412, 609, 314, 461, 363),
                        TimeOnAirData(7, 7, 2, True, True, False, 8, False, "RAK3172", 231, 354, 157, 255, 206),
                        TimeOnAirData(6, 7, 2, True, True, False, 8, False, "RAK3172", 144, 218, 95, 144, 120),
                        TimeOnAirData(5, 7, 2, True, True, False, 8, False, "RAK3172", 85, 134, 54, 91, 72),
                        
                        TimeOnAirData(6, 7, 2, False, True, False, 8, False, "RAK3172", 107, 157, 83, 120, 95),
                        TimeOnAirData(5, 7, 2, False, True, False, 8, False, "RAK3172", 60, 91, 42, 66, 54),

                        # RfBW = 7 => 31,25 kHz on RAK3172, CodeRate 3 = 4/7
                        TimeOnAirData(12, 7, 3, True, True, False, 8, False, "RAK3172", 5410, 7240, 3570, 5410, 4490),
                        TimeOnAirData(11, 7, 3, True, True, False, 8, False, "RAK3172", 2700, 4080, 2240, 3160, 2240),
                        TimeOnAirData(10, 7, 3, True, True, False, 8, False, "RAK3172", 1580, 2270, 1120, 1580, 1350),
                        TimeOnAirData(9, 7, 3, True, True, False, 8, False, "RAK3172", 791, 1250, 562, 906, 676),
                        TimeOnAirData(8, 7, 3, True, True, False, 8, False, "RAK3172", 453, 682, 338, 510, 396),
                        TimeOnAirData(7, 7, 3, True, True, False, 8, False, "RAK3172", 255, 399, 169, 284, 227),
                        TimeOnAirData(6, 7, 3, True, True, False, 8, False, "RAK3172", 161, 247, 103, 161, 132),
                        TimeOnAirData(5, 7, 3, True, True, False, 8, False, "RAK3172", 95, 152, 59, 102, 81),

                        TimeOnAirData(6, 7, 3, False, True, False, 8, False, "RAK3172", 118, 175, 89, 132, 103),
                        TimeOnAirData(5, 7, 3, False, True, False, 8, False, "RAK3172", 66, 102, 45, 73, 59),

                        # RfBW = 7 => 31,25 kHz on RAK3172, CodeRate 4 = 4/8
                        TimeOnAirData(12, 7, 4, True, True, False, 8, False, "RAK3172", 5800, 7900, 3700, 5800, 4750),
                        TimeOnAirData(11, 7, 4, True, True, False, 8, False, "RAK3172", 2900, 4470, 2380, 3420, 2380),
                        TimeOnAirData(10, 7, 4, True, True, False, 8, False, "RAK3172", 1710, 2500, 1190, 1710, 1450),
                        TimeOnAirData(9, 7, 4, True, True, False, 8, False, "RAK3172", 857, 1380, 594, 988, 725),
                        TimeOnAirData(8, 7, 4, True, True, False, 8, False, "RAK3172", 494, 756, 363, 560, 428),
                        TimeOnAirData(7, 7, 4, True, True, False, 8, False, "RAK3172", 280, 444, 181, 313, 247),
                        TimeOnAirData(6, 7, 4, True, True, False, 8, False, "RAK3172", 177, 275, 112, 177, 144),
                        TimeOnAirData(5, 7, 4, True, True, False, 8, False, "RAK3172", 105, 171, 64, 113, 89),

                        TimeOnAirData(6, 7, 4, False, True, False, 8, False, "RAK3172", 128, 193, 95, 144, 112),
                        TimeOnAirData(5, 7, 4, False, True, False, 8, False, "RAK3172", 72, 113, 48, 81, 64),
                        ...
                        TimeOnAirData(6, 7, 1, True, True, False, 8, True, "RAK3172", 128, 189, 87, 138, 107),
                        TimeOnAirData(5, 7, 1, True, True, False, 8, True, "RAK3172", 80, 121, 54, 85, 64),

                        TimeOnAirData(6, 7, 1, False, True, False, 8, True, "RAK3172", 107, 148, 77, 107, 87),
                        TimeOnAirData(5, 7, 1, False, True, False, 8, True, "RAK3172", 59, 85, 44, 59, 49),
                        # RfBW = 7 => 31,25 kHz on RAK3172, CodeRate 2 = 4/6
                        ...
                        TimeOnAirData(6, 7, 2, True, True, False, 8, True, "RAK3172", 144, 218, 95, 157, 120),
                        TimeOnAirData(5, 7, 2, True, True, False, 8, True, "RAK3172", 91, 140, 60, 97, 72),

                        TimeOnAirData(6, 7, 2, False, True, False, 8, True, "RAK3172", 120, 169, 83, 120, 95),
                        TimeOnAirData(5, 7, 2, False, True, False, 8, True, "RAK3172", 66, 97, 48, 66, 54),
                        ...
                        TimeOnAirData(6, 7, 3, True, True, False, 8, True, "RAK3172", 161, 247, 103, 161, 132),
                        TimeOnAirData(5, 7, 3, True, True, False, 8, True, "RAK3172", 102, 159, 66, 102, 81),

                        TimeOnAirData(6, 7, 3, False, True, False, 8, True, "RAK3172", 132, 189, 89, 132, 103),
                        TimeOnAirData(5, 7, 3, False, True, False, 8, True, "RAK3172", 73, 109, 52, 73, 59),
                        ...
                        TimeOnAirData(6, 7, 4, True, True, False, 8, True, "RAK3172", 177, 275, 112, 193, 144),
                        TimeOnAirData(5, 7, 4, True, True, False, 8, True, "RAK3172", 113, 179, 72, 122, 89),

                        TimeOnAirData(6, 7, 4, False, True, False, 8, True, "RAK3172", 144, 210, 95, 144, 112),
                        TimeOnAirData(5, 7, 4, False, True, False, 8, True, "RAK3172", 81, 122, 56, 81, 64),
